...
class_weights = (y_train.sum(dim=0) / 1000).to(torch.float)
# instantiate model
bert_model_wt = BERTClass()
criterion = nn.CrossEntropyLoss(weight=class_weights)
optimizer = optim.Adam(bert_model_wt.parameters(), lr=0.00002)
# run training loop
train_loss_list, val_loss_list = bert_training_loop(bert_model=bert_model_wt, criterion=criterion, 
...
plt.savefig(f'jobs/bert_fine_tuning_weighted.png')

# save trained model
torch.save(bert_model_wt.state_dict(), f'jobs/bert_fine_tuned_weighted.pt')